...
nm = 1e-9
lda = linspace(210, 27000, 1000) # list of wavelengths in nm
...
d_list = [inf, 2000, 4000,2000, 200, inf] # list of layer thicknesses in nm # 500nm Al2O3 good
c_list = ['i', 'c', 'c','c','c','i']
theta = 0
T_list = [];
R_list = [];
...
for lda0 in lda:
    ...
    n_list = [1,  msio2np_ideal_fn(lda0), msio2_fn(lda0), mal2o3np_fn(lda0), mag_fn(lda0), 1]
...
for lda0 in lda:
    ...
    n_list = [1, mal2o3np_fn(lda0), msio2np_ideal_fn(lda0), msio2_fn(lda0), mag_fn(lda0), 1]
    inc_tmm_data = tmm.inc_tmm('s',n_list,d_list,c_list,theta,lda0)
    A_list.append(tmm.inc_absorp_in_each_layer(inc_tmm_data)) #stores as list of np.arrays
    T_list.append(inc_tmm_data['T'])
    R_list.append(inc_tmm_data['R'])    
