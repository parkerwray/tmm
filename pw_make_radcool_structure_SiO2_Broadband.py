...
m = datalib.alloy(lda*nm, 0.30, 'Air','RC0_1D_Al2O3','Bruggeman') # 15% ff good
...
#d_list = [inf, 1500, 3000, 200, inf] # list of layer thicknesses in nm # 500nm Al2O3 good

d_list = [inf, 2500, 3500, 200, inf] # list of layer thicknesses in nm # 500nm Al2O3 good
...
for lda0 in lda:
    ...
    n_list = [1,msio2np_ideal_fn(lda0),msio2_fn(lda0),  mag_fn(lda0), 1]
...
Ai = stack(A_list, axis = 0) # convert list of np.arrays to single np.array
Ti = array(T_list, dtype = complex) # Convert list to array for math operations
Ri = array(R_list, dtype = complex) # Convert list to array for math operations
...
#plt.plot(lda[mask]*1e-3, (1-Ti[mask]-Ri[mask])*100,'r--', label = 'Device absorption \n (Incoherent)')
#plt.plot(lda[mask]*1e-3, A[mask,1]*100,':', label = 'Abs. $Al_{2}O_{3}$ NP \n (15%, Brugg.)')    
plt.plot(lda[mask]*1e-3, A[mask,1]*100,':', label = 'Abs. $SiO_{2}$ NP \n (30%, Brugg.)')
plt.plot(lda[mask]*1e-3, A[mask,2]*100,':', label = 'Abs. $SiO_{2}$')
plt.plot(lda[mask]*1e-3, A[mask,3]*100,':', label = 'Abs. $Ag$')
...
plt.plot(lda[mask], (1-Ti[mask]-Ri[mask])*100,'r--', label = 'Device absorption \n (Incoherent)')
#plt.plot(lda[mask], A[mask,1]*100,':', label = 'Abs. $Al_{2}O_{3}$ NP \n (15%, Brugg.)')    
plt.plot(lda[mask], A[mask,1]*100,':', label = 'Abs. $SiO_{2}$ NP \n (30%, Brugg.)')
plt.plot(lda[mask], A[mask,2]*100,':', label = 'Abs. $SiO_{2}$')
