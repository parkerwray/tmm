...
def animate(i):
    ...
    plt.plot(rad.lda*1e6,rad.BBml, label = 'Structure emmisivity')
    ln1.set_data(rad.lda*1e6, rad.e_struct)    
    ax1.collections.clear()
    fill1 = plt.fill_between(rad.lda*1e6,0,rad.e_amb,color = 'b', alpha=0.5)
    fill2 = plt.fill_between(rad.lda*1e6,rad.e_amb,rad.e_struct,
                     where =rad.e_struct > rad.e_amb, alpha=1)      
...
ani = animation.FuncAnimation(fig, animate, frames=len(T), init_func = init,  repeat=True)
