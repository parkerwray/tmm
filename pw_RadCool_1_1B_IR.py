...
Rref_list = [];
Aref_list = [];
dref_list = [inf, 0.848, 525000, inf] # list of layer thicknesses in nm %Ellip shows 15.62 SiO2 native oxide layer
cref_list = ['i', 'c', 'i', 'i']
theta = 0
for lda0 in lda:
...
#plt.plot(lda, (1-np_T-np_R)*100,'r', label = 'Measured structure absorption')
#
#plt.xlabel('Wavelength (nm)')
#plt.ylabel('%')
#plt.title('Transmission, reflection, and absorption at normal incidence')
...
if (min(lda) > 2000):
    ...
    plt.plot(lda*1e-3, Aideal[:,1]*100,'r:', label = 'Nanoparticle layer \n (24.5% $Si_3N_4$ Brugg.)')
    ...
    plt.plot(lda, Aideal[:,1]*100,'k:', label = 'Nanoparticle layer \n (24.5% $Si_3N_4$ Brugg.)')
    plt.plot(lda, Aideal[:,3]*100,'m:', label = 'Si Substrate')
